  ...
  @parse : ({key_bundles}, cb) ->
    ...
    for bundle in key_bundles
      ...
      if (existing = kids_to_key_managers[kid_str])?
        # We may get multiple versions of a PGP key with the same
        # KID/fingerprint. They could have different subkeys, and it could be
...
class ChainLink
  @parse : ({sig_blob, parsed_keys, sig_cache}, cb) ->
    ...
    await @_check_payload_against_server_kid {sig_blob, payload, parsed_keys}, esc defer()
  ...
  @_check_payload_against_server_kid : ({sig_blob, payload, parsed_keys}, cb) ->
    # Here's where we check the data we relied on in @_unbox_payload().
    signing_kid = sig_blob.kid
    signing_fingerprint = parsed_keys.get_key_manager(signing_kid).get_pgp_fingerprint()?.toString('hex')
...
exports.SigChain = class SigChain
  ...
  get_sibkeys : ({now}) ->
    ...
    for kid in @_sibkey_order
      ...
      if @_valid_sibkeys[kid] and not expired
        ret.push(@_parsed_keys.get_key_manager kid)
    ret
  ...
  _update_kid_pgp_etime : ({kid}) ->
    ...
    key_manager = @_parsed_keys.get_key_manager kid
  ...
  _enforce_eldest_key_ownership : ({}, cb) ->
    ...
    eldest_km = @_parsed_keys.get_key_manager @_eldest_kid
    if not eldest_km?
