...
(function() {
  ...
  exports.GenericWebSiteProofGen = GenericWebSiteProofGen = (function(_super) {
    ...
    GenericWebSiteProofGen.prototype.filename = function() {
      var file;
      file = proofs.GenericWebSiteScraper.FILE;
      return this.remote_host + "/" + file;
    };
    ...
    GenericWebSiteProofGen.prototype.get_warnings = function(_arg) {
      var b, remote_name_normalized, ret;
      remote_name_normalized = _arg.remote_name_normalized;
      ret = [];
      if (remote_name_normalized.indexOf("https://") >= 0) {
        ret = ["HTTPS proofs won't work for self-signed certificates;", "Only proceed if your cert is signed by a well-known CA"];
      } else {
        b = colors.bold;
        ret = ["" + (b('HTTPS')) + " websites are prefered, since they prove control of DNS records " + (colors.bold("and")) + " SSL certs;", "Proceed with proving an HTTP host if your site doesn't support HTTPS"];
      }
      return ret;
    };
