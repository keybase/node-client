...
(function() {
  ...
  exports.Link = Link = (function() {
      ...
      (function(_this) {
        return (function(__iced_k) {
          __iced_deferrals = new iced.Deferrals(__iced_k, {
            ...
            filename: "/home/max/src/keybase/node-client/src/chainlink.iced",
            funcname: "Link.store"
          });
          db.put({
            type: Link.ID_TYPE,
    ...
    Link.prototype._perform_revocations = function(_arg) {
      var id, ids, index, r, table, _i, _len, _ref3;
      table = _arg.table, index = _arg.index;
      ids = [];
      if ((r = (_ref3 = this.body()) != null ? _ref3.revoke : void 0) != null) {
        if (r.sig_id != null) {
          ids.push(r.sig_id);
        }
        if (r.sig_ids != null) {
          ids = ids.concat(r.sig_ids);
        }
      }
      if (ids.length) {
        ...
        for (_i = 0, _len = ids.length; _i < _len; _i++) {
          id = ids[_i];
          this._perform_revocation({
            id: id,
            table: table,
            index: index
          });
        }
        return log.debug("- Link::_perform_revocations");
      }
    };

    Link.prototype._perform_revocation = function(_arg) {
      var id, index, link, table;
      id = _arg.id, table = _arg.table, index = _arg.index;
      log.debug("+ Link::_perform_revocation " + id);
      if ((link = index[id]) == null) {
      ...
      return log.debug("- Link::_perform_revocation " + id);
    };
  ...
  RemoteProof = (function(_super) {
    ...
    RemoteProof.prototype.check_remote_proof = function(_arg, cb) {
      var arg, e2, err, errmsg, esc, msg, ok, proof_vec, pubkey, rc, rsc, scraper, skip, type, type_s, username, warnings, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
      skip = _arg.skip, pubkey = _arg.pubkey, type = _arg.type, warnings = _arg.warnings, proof_vec = _arg.proof_vec;
